...
public struct Tile : IBodyComponent
{
    ...
    public Tile(int x, int y, int width, int height, byte id, float layerDepth) 
    {
    ...
    public Tile(Point location, Point size, byte id, float layerDepth) : this(location.X, location.Y, size.X, size.Y, id, layerDepth) {}
    
    public Tile(Rectangle bounds, byte id, float layerDepth, Color color) : this(bounds.Location, bounds.Size, id, layerDepth) {}
}